...
这个文件提供了SHN社工库所需要的主要和简单操作，这些操作是组成SHN社工库的根本，有了这些操作，
SHN社工库才可以正常运转。

作为一个社工库，一定需要有大致分为三方面的操作：读取、添加和修改。一个社工库的运转需要如下的
步骤：在现实生活中获取信息->添加人物画像中的信息->在之后读取人物的信息->及时修改信息。在
第二步中，需要提供指定的项目信息，每个项目都在程序本身的函数中有所体现；在第四步中，程序可以
在某些情况下自动更新有需要的信息，比如距离生日的天数等。
...
class Person:
    ...
    在这个类中，拥有添加、读取和修改的函数以提供社工库的基本操作，也拥有基本的信息模板
    ...
    def __init__(self, fromf=True, **kwargs):
        ...
        可以从文件内导入或者新建画像。如果想从文件导入，fromf应为True，反之则为False。
        当fromf为True时，需要提供人物画像的shnid唯一编号；当fromf为False时，需要提供一系列的
        基本信息（具体参照上方基本信息模板）。
